def define_env(env):
  ...
  def email(address: str, domain: str = "consensys.net"):
    return 'Send email at [{address}@{domain}](mailto:{address}@{domain})'.format(address=address,
                                                                                  domain=domain)
  # This is a demo macro that you can define for all your site
  @env.macro
  def color_block(value: str):
    return '<span style="border:solid 1px black; border-right:none; ' \
  ...
  def cli_to_env(name: str, prefix: str = ""):
    return (prefix + ('_' if prefix else '') + name).replace("--", "").replace("-", "_").upper()
